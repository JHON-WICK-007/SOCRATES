...
def call_claude(prompt: str, system: str = None) -> str:
    ...
    """
    # 1. Try direct HTTP proxy call if ANTHROPIC_BASE_URL is set (avoids subprocess/CLI issues entirely)
    ...
    if base_url:
        try:
            ...
            # Use proxy configured model or default
            model = os.environ.get("CAVEMAN_MODEL")
            if not model:
                # Resolve active model from env/proxy settings if possible
            ...
            import sys
            sys.stderr.write("Starting direct proxy call...\n")
            sys.stderr.flush()
            compressed_text = ""
                ...
                while True:
                    ...
                    if line_str.startswith("data:"):
                        ...
                        try:
                            ...
                            pass
            if compressed_text:
                return strip_llm_wrapper(compressed_text.strip())
        except Exception as e:
            import sys
            sys.stderr.write(f"Direct proxy call failed: {e}\n")
            sys.stderr.flush()
...
def build_compress_system_prompt() -> str:
    return """You are a markdown compression agent. Your job is to compress the provided natural language text into caveman format.

STRICT RULES:
- Do NOT delete, omit, or modify any ``` code blocks. Every code block in the original text must exist in the compressed text exactly as-is.
- Do NOT delete, omit, or modify any inline backticks (e.g. `code`). Every inline code block in the original text must exist in the compressed text exactly as-is.
- Do NOT delete, omit, or modify any URLs. Every URL in the original text must exist in the compressed text exactly as-is.
- Do NOT delete, omit, or modify any headings. Every heading in the original text must exist in the compressed text exactly as-is.
...
def build_fix_system_prompt(errors: List[str]) -> str:
    errors_str = "\n".join(f"- {e}" for e in errors)
    return f"""You are fixing a caveman-compressed markdown file. Specific validation errors were found.

CRITICAL RULES:
- DO NOT recompress or rephrase the file.
- ONLY fix the listed errors — leave everything else exactly as-is. Do not alter any other text, newlines, or structure.
- Preserve caveman style in all untouched sections.
- Return ONLY the raw fixed markdown content. Do NOT wrap the output in a ```markdown fence or any other block fence.
...
def build_fix_prompt(original: str, compressed: str) -> str:
    return f"ORIGINAL (reference only):\n{original}\n\nCOMPRESSED (fix this):\n{compressed}"
...
def compress_file(filepath: Path) -> bool:
    ...
    compressed_body = call_claude(
        build_compress_prompt(body),
        system=build_compress_system_prompt()
    )
    ...
    try:
        for attempt in range(MAX_RETRIES):
            ...
            compressed = call_claude(
                build_fix_prompt(original_text, compressed),
                system=build_fix_system_prompt(result.errors)
            )
            filepath.write_text(compressed, encoding="utf-8")
    except Exception as e:
        import traceback
